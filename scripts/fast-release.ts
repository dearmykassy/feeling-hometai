...
async function generateBrowserQa(context: FastContext) {
  ...
  const chromeProfile = await mkdtemp(
    resolve(tmpdir(), "feeling-hometai-fast-chromium-"),
  );
  ...
  try {
    ...
    const report = {
      schemaVersion: FAST_RELEASE_CONTRACT.localChromiumQaSchema,
...
async function assertBrowserQa(context: FastContext, report: UnknownRecord) {
  ...
  if (
    report.schemaVersion !== FAST_RELEASE_CONTRACT.localChromiumQaSchema ||
    report.status !== "PASS" ||
    report.platformId !== FAST_RELEASE_CONTRACT.platformId ||
...
async function buildReceipt(context: FastContext) {
  ...
  return {
    schemaVersion: FAST_RELEASE_CONTRACT.candidateReceiptSchema,
