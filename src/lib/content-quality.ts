...
export const CONTENT_NORMALIZER_VERSION = "feeling-hometai-location-number-count-strip/v1";
...
function normalizeNgramSurface(value: string): string {
  return normalizeRegionalText(value)
    .replace(/(?:필링홈타이|필링이라는|필링에게|feeling hometai)/giu, " ")
    .replace(/\s+/gu, " ")
    .trim();
}
...
function buildCuratedReuseAudit(
  ...
  return {
    policyVersion:
      options.policyVersion ?? "feeling-hometai-curated-complete-sentence-family-reuse/v1",
    rationale:
      options.rationale ??
...
function buildSeoCopyBankAudit(contents: readonly RegionContent[]) {
  ...
  const banks = definitions.map(({ familyId, bank, value }) => {
    ...
    for (const [index, content] of contents.entries()) {
      ...
      if (familyId === "title") {
        const brandCount = occurrences(actual, "필링홈타이");
  ...
  return {
    ...
    rules: [
      ...
      "title은 60자 이하이고 필링홈타이를 정확히 한 번 포함합니다.",
...
function buildSentenceSimilarityAudit(contents: readonly RegionContent[]) {
  ...
  for (const [index, sentenceMap] of sentenceMaps.entries()) {
    ...
    if (missing.length > 0 || extra.length > 0) {
      throw new Error(
        `FEELING_HOMETAI_SENTENCE_SIMILARITY_SHAPE:${ACTIVE_REGION_NODES[index].path}:${missing.join(",")}:${extra.join(",")}`,
      );
    }
  }
...
export function buildDiversityAudit(contents: readonly RegionContent[]) {
  ...
  const normalizedDocumentReuse = buildCuratedReuseAudit(
    ...
    128,
    {
      policyVersion: "feeling-hometai-normalized-template-diagnostic/v1",
      rationale:
