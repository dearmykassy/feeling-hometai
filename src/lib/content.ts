...
export const CURATED_TITLE_BANK = regionalSeoBank(
  (label) => `${label} 필링홈타이 | 주소·시간·코스 이용 안내`,
  (label) => `${label} 필링홈타이 | 코스와 시간을 고르는 이용 안내`,
  (label) => `${label} 필링홈타이 | 가격표부터 현장 결제까지`,
  (label) => `${label} 필링홈타이 | 원하는 일정에 맞춘 홈타이`,
  (label) => `${label} 필링홈타이 | 코스·가격·시작 시간 안내`,
  (label) => `${label} 필링홈타이 | 서비스 주소와 희망 시각 상담`,
  (label) => `${label} 필링홈타이 | 24시간 이용 일정 상담`,
  (label) => `${label} 필링홈타이 | 선입금 없는 현장 후불 안내`,
  (label) => `${label} 필링홈타이 | 코스별 시간과 가격표 확인`,
  (label) => `${label} 필링홈타이 | 서비스 전 주소·일정 준비`,
  (label) => `${label} 필링홈타이 | 현장 카드 결제와 코스 안내`,
);

export const CURATED_H1_BANK = regionalSeoBank(
  (label) => `${label}, 원하는 시간에 맞춘 필링홈타이`,
  (label) => `${label} 일정에 맞춰 고르는 필링홈타이`,
  (label) => `${label} 서비스 전에 확인하는 코스와 시간`,
  (label) => `${label}, 서비스를 받을 주소와 희망 시각을 맞추는 필링홈타이`,
  (label) => `${label}, 하루 일정에 맞춘 코스와 시작 시간`,
  (label) => `${label}의 주소와 일정에 맞춘 필링홈타이`,
  (label) => `${label}, 코스부터 현장 결제까지 한눈에`,
  (label) => `${label} 서비스에 필요한 주소·시간·코스 안내`,
  (label) => `${label}에서 준비하는 나만의 홈타이 일정`,
...
> = {
  description: regionalSentenceBank(
    (_node, label) => `${label} 필링홈타이 안내에서는 서비스를 받을 정확한 주소와 희망 시각, 코스별 시간과 표시 금액, 24시간 전화상담, 선입금 없는 현장 후불과 카드 결제 기준을 한눈에 살펴볼 수 있습니다`,
    (_node, label) => `${label} 필링홈타이 이용 안내에는 도로명과 건물명, 원하는 시작 시각, 코스별 소요 시간과 표시 금액, 24시간 전화상담, 현장 후불과 카드 결제 기준이 정리되어 있습니다`,
    (_node, label) => `${label} 필링홈타이는 서비스를 받을 정확한 주소와 연락받을 고객 번호를 준비하는 방법, 관리 방식별 코스 시간과 표시 금액, 24시간 전화상담, 선입금 없는 현장 결제 원칙을 간결하게 안내합니다`,
    (_node, label) => `${label} 필링홈타이 소개에는 도로명 주소와 희망 시작 시각, 이용 인원, 코스별 소요 시간과 예산, 당일 가능 여부를 묻는 24시간 전화상담, 예약금 없는 후불과 카드 결제 기준이 담겨 있습니다`,
    (_node, label) => `${label} 필링홈타이 이용 안내에서는 도로명 주소와 건물명, 희망 시각, 관리 방식별 가격표, 24시간 전화상담, 선입금 없는 현장 후불·카드 결제 정보를 확인할 수 있습니다`,
    (_node, label) => `${label} 필링홈타이 방문 정보에는 코스 종류와 소요 시간, 표시 금액, 상담 전에 준비할 서비스 주소와 연락받을 고객 번호, 당일 운영 여부, 이용 후 현장 결제 원칙이 보기 쉽게 정리되어 있습니다`,
    (_node, label) => `${label} 필링홈타이에서는 관리 방식별 코스와 가격표, 요청한 주소와 희망 시각의 서비스 가능 여부, 24시간 전화상담, 예약금 없는 현장 후불과 카드 결제 기준을 차례로 소개합니다`,
    (_node, label) => `${label} 필링홈타이를 알아보는 분을 위해 건물명과 도로명, 시작 시각, 코스별 시간과 예산, 당일 가능 여부를 묻는 상담, 선입금 없는 후불 및 카드 결제 기준을 보기 쉽게 정리했습니다`,
    (_node, label) => `${label} 필링홈타이 안내에는 서비스 주소·이용 인원·연락받을 고객 번호, 관리 방식별 코스 시간과 표시 금액, 당일 가능 여부를 묻는 상담, 선입금 없는 현장 후불과 카드 결제 기준이 담겨 있습니다`,
    (_node, label) => `${label} 필링홈타이 소개에서 코스별 관리 방식과 가격, 서비스를 받을 정확한 주소와 희망 시각, 당일 가능 여부를 묻는 24시간 전화상담, 예약금 없는 현장 결제 정보를 한눈에 확인하세요`,
    (_node, label) => `${label} 필링홈타이의 가격표와 관리 방식별 코스, 서비스 전 확인할 주소와 시작 시각, 당일 운영 여부, 24시간 전화상담, 선입금 없는 현장 후불과 카드 결제 기준을 안내합니다`,
  ),
...
function titleFor(keywordLabel: string): string {
  return `${keywordLabel}홈타이 ${keywordLabel}출장마사지 | 필링홈타이`;
}
...
function createRegionContentInternal(node: RegionNode): RegionContent {
  ...
  return {
    ...
    eyebrow: "FEELING HOMETAI · PRIVATE CARE",
