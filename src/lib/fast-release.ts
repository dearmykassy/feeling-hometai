...
export const FAST_RELEASE_CONTRACT = {
...
export type FastCandidate = {
  protocol: typeof FAST_RELEASE_CONTRACT.candidateBindingProtocol;
  id: string;
  platformId: typeof FAST_RELEASE_CONTRACT.platformId;
  corpusSha256: string;
...
function assertBuiltAudit(corpus: UnknownRecord, audit: UnknownRecord, corpusBuffer: Buffer) {
  ...
  if (
    audit.schemaVersion !== FAST_RELEASE_CONTRACT.builtOutputAuditSchema ||
    audit.status !== "PASS" ||
    audit.platformId !== FAST_RELEASE_CONTRACT.platformId ||
...
function assertImageReleaseBoundary(corpus: UnknownRecord) {
  ...
  if (
    ...
    throw new Error("FEELING_HOMETAI_FAST_IMAGE_RELEASE_BOUNDARY");
  }
  if (
    receipt.source !== "FEELING_IMAGE_RELEASE_RECEIPT" ||
    ...
    throw new Error("FEELING_HOMETAI_FAST_IMAGE_RELEASE_RECEIPT_BINDING");
  }
}
...
export async function loadFastContext(root: string): Promise<FastContext> {
  ...
  const material = {
    protocol: FAST_RELEASE_CONTRACT.candidateBindingProtocol,
    platformId: FAST_RELEASE_CONTRACT.platformId,
...
export function buildFastAiReview(context: FastContext) {
  ...
  return {
    schemaVersion: FAST_RELEASE_CONTRACT.aiCopyReviewSchema,
