...
export function createRegionPageModel(node: RegionNode): RegionPageModel {
  ...
  if (content.hooks.length !== 2 || content.ctaLabels.length !== 3) {
    throw new Error(`FEELING_HOMETAI_REGION_COPY_SHAPE_INVALID:${node.path}`);
  }
  ...
  if (headingViolations.length > 0) {
    throw new Error(
      `FEELING_HOMETAI_REGION_ADJACENT_HEADING_SEMANTIC_DUPLICATE:${node.path}:${headingViolations[0].actionId}`,
    );
  }
  ...
  if (new Set(ids).size !== ids.length) {
    throw new Error(`FEELING_HOMETAI_REGION_RENDER_COPY_ID_DUPLICATE:${node.path}`);
  }
