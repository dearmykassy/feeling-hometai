import regionalImageAssignmentsJson from "@/data/regional-image-assignments.template3.generated.json";
...
const ASSET_ID = /^rng-t3-rgn-\d{3}-v1$/;
const IMAGE_VARIANTS: readonly ImageVariant[] = ["desktop", "tablet", "mobile"];

function fail(code: string): never {
  throw new Error(`FEELING_HOMETAI_REGIONAL_IMAGE_RUNTIME_${code}`);
}
...
function validateAssignment(route: string, assignment: RegionalImageAssignment): void {
  ...
  for (const variant of IMAGE_VARIANTS) {
    const expected = `/assets/feeling-hometai/template3-regional/${assignment.assetId}/${variant}.webp`;
