...
if (
  ...
  throw new Error("FEELING_HOMETAI_CAPITAL_REGION_SOURCE_INVALID");
}
...
if (
  ...
  throw new Error("FEELING_HOMETAI_SERVICE_REGION_SOURCE_INVALID");
}
...
function nodeForSegments(segmentsInput: readonly string[]): RegionNode | null {
  ...
  return {
    id: `feeling-hometai-region-${segments.map(encodeURIComponent).join("-")}`,
    kind,
    rootKey,
    segments,
...
if (ACTIVE_REGION_NODES.length !== 1291) {
  throw new Error(
    `FEELING_HOMETAI_ACTIVE_ROUTE_COUNT_INVALID:${ACTIVE_REGION_NODES.length}`,
  );
}
...
if (new Set(SEARCH_REGION_LABELS).size !== ACTIVE_REGION_NODES.length) {
  throw new Error("FEELING_HOMETAI_SEARCH_REGION_LABEL_DUPLICATE");
}
...
export function getRootNode(rootKey: ActiveRootKey): RegionNode {
  ...
  if (!node) throw new Error(`FEELING_HOMETAI_ROOT_NOT_FOUND:${rootKey}`);
  return node;
}
