...
describe("feeling-hometai content corpus", () => {
  ...
  it("keeps documents and paragraphs exact-unique while limiting reviewed sentence reuse to the same fact slot", () => {
    ...
    expect(audit).toMatchObject({
      normalizer: "feeling-hometai-location-number-count-strip/v1",
      rawDocuments: {
      ...
      visibleSentenceReusePolicy: {
        policyVersion: "feeling-hometai-visible-exact-section-slot-bank/v1",
      ...
      regionalSentenceBanks: {
        policyVersion: "feeling-hometai-region-first-complete-sentence-bank/v1",
        acceptedMaximumFrequency: 128,
        bankCount: 15,
      ...
      seoCopyBanks: {
        policyVersion: "feeling-hometai-seo-title-h1-complete-copy-bank/v1",
        acceptedMaximumFrequency: 128,
        bankCount: 2,
        templateCount: 22,
        maximumFrequency: 118,
  ...
  it("pins unique human SEO fields and every owned keyword head on every route", () => {
    ...
    for (const document of corpus.documents) {
      ...
      expect(document.keywords.join(" ")).not.toMatch(/남성전용/u);
      expect(
        folioPattern.test(
    ...
    expect(corpus.fixedDocuments.every((document) =>
      ...
      (document.title.match(/필링홈타이/gu) ?? []).length === 1,
    )).toBe(true);
  ...
  it("binds the approved regional image release while keeping public deployment blocked", () => {
    const releaseReceiptBuffer = readFileSync(
      resolve("artifacts/image-release/feeling-hometai-template3-regional-release.v1.json"),
    );
    const assignmentManifestBuffer = readFileSync(
      resolve("src/data/regional-image-assignments.template3.generated.json"),
    );
    ...
    expect(imageState.receipt).toMatchObject({
      ...
      assignmentManifest: {
        relativePath: "src/data/regional-image-assignments.template3.generated.json",
