...
describe("feeling hometai fixed pages and blog", () => {
  ...
  it("makes the two original blog notes reachable and schema-bound", () => {
    ...
    for (const post of BLOG_POSTS) {
      ...
      expect(post.image.src).toMatch(/^\/images\/feeling-template3\/blog\/feeling-note-0[12]-v1\.webp$/u);
      expect(existsSync(`public${post.image.src}`)).toBe(true);
    }

    const provenance = JSON.parse(
      readFileSync("public/images/feeling-template3/blog/provenance.json", "utf8"),
    ) as { assets: { assetId: string; webp: { path: string; width: number; height: number } }[] };
    expect(provenance.assets.map((asset) => asset.assetId)).toEqual(
      BLOG_POSTS.map((post) => post.image.assetId),
    );
    expect(provenance.assets.every((asset) =>
      asset.webp.width === 1600 &&
      asset.webp.height === 900 &&
      existsSync(asset.webp.path)
    )).toBe(true);
  });
